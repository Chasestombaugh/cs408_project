...
test.describe('Matches Page', () => {
  ...
  test('should display match detail page for a seeded SR match', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('13,117');
    await expect(page.locator('body')).toContainText('57');
    await expect(page.locator('body')).toContainText('20,611');
    await expect(page.locator('body')).toContainText('16,362');
  });
  ...
  test('should display username-filtered stats with derived metrics', async ({ page }) => {
    ...
    await expect(page.locator('body')).toContainText('KDA');
    await expect(page.locator('body')).toContainText('Average Stats');
    ...
    await expect(page.locator('body')).toContainText('Damage Dealt');
    await expect(page.locator('body')).toContainText('Damage Taken');
  });

  test('should navigate from stats champion link to champion-filtered matches', async ({ page }) => {
    await page.goto('/debug/seed');
    await page.goto('/stats?user=TravisSqrt&range=all');
    ...
    await expect(page.locator('body')).toContainText('TravisSqrt');
    await expect(page.locator('body')).toContainText('Champion:');
    await expect(page.locator('body')).toContainText('Twitch');

    await expect(page.locator('tbody tr')).toHaveCount(2);
    await expect(page.locator('tbody')).toContainText('Twitch');
  });

  test('should display champion-filtered stats when clicking a champion name', async ({ page }) => {
    await page.goto('/debug/seed');
    await page.goto('/stats?user=TravisSqrt&range=all');

    await page.getByRole('link', { name: 'Twitch' }).first().click();

    await expect(page).toHaveURL(/\/stats\?user=TravisSqrt&champion=Twitch&range=all/);
    await expect(page.locator('body')).toContainText('Viewing stats for:');
    await expect(page.locator('body')).toContainText('TravisSqrt');
    await expect(page.locator('body')).toContainText('Champion:');
    await expect(page.locator('body')).toContainText('Twitch');
    await expect(page.locator('body')).toContainText('Clear filter');
